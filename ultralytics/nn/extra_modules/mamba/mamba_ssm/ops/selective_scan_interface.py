...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def backward(ctx, doubt):
        ...
        # dout_y = rearrange(doubt, "b l d -> b d l") # because no arrange at end of forward, so doubt shape is b d l
...
class MambaInnerFn(torch.autograd.Function):
    ...
    def backward(ctx, doubt):
        ...
        dxz = torch.empty_like(xz)  # (batch, dim, seqlen)
        dx, dz = dxz.chunk(2, dim=1)
        doubt = rearrange(doubt, "b l e -> e (b l)")
        dout_y = rearrange(out_proj_weight.t() @ doubt, "d (b l) -> b d l", l=L)
        dconv1d_out, ddelta, dA, dB, dC, dD, ddelta_bias, dz, out_z = selective_scan_cuda.bwd(
            conv1d_out,
            delta,
            A,
        ...
        dout_proj_weight = torch.einsum("eB,dB->ed", doubt, rearrange(out_z, "b d l -> d (b l)"))
        dout_proj_bias = doubt.sum(dim=(0, 1)) if not ctx.out_proj_bias_is_None else None
        dD = dD if D is not None else None
        dx_dbl = torch.empty_like(x_dbl)
        dB_proj_bias = None
        if ctx.is_variable_B:
