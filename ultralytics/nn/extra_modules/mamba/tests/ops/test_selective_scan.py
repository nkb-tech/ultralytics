...
def test_bimamba_inner_fn_grad_check(is_variable_B, is_variable_C, seqlen, itype, wtype):
    ...
    D = torch.randn(dim, device=device, dtype=torch.float32, requires_grad=True)
    delta_bias = (0.5 * torch.rand(dim, device=device, dtype=torch.float32)).requires_grad_()
    xz.detach().clone().requires_grad_()
    conv1d_weight.detach().clone().requires_grad_()
    conv1d_bias.detach().clone().requires_grad_()
    x_proj_weight.detach().clone().requires_grad_()
    delta_proj_weight.detach().clone().requires_grad_()
    out_proj_weight.detach().clone().requires_grad_()
    out_proj_bias.detach().clone().requires_grad_() if out_proj_bias is not None else None
    A.detach().clone().requires_grad_()
    A_b.detach().clone().requires_grad_()
    B.detach().clone().requires_grad_() if B is not None else None
    C.detach().clone().requires_grad_() if C is not None else None
    D.detach().clone().requires_grad_()
