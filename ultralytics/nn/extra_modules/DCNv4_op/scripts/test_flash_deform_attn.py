...
from functions import FlashDeformAttnFunction, MSDeformAttnFunction
...
def check_forward_equal_with_pytorch_half():
    ...
    sampling_loc_attn = torch.cat(
        [sampling_locations.reshape(N, Lq, M, L * P * 2), attention_weights.reshape(N, Lq, M, L * P)], dim=-1
    )
    attention_weights = torch.nn.functional.softmax(attention_weights.flatten(-2, -1), dim=-1).unflatten(-1, (L, P))

    im2col_step = 128

    flash_fn_args = (value.half(), shapes, level_start_index, sampling_loc_attn.half(), im2col_step, P, 16)
    output_cuda = (FlashDeformAttnFunction.apply(*flash_fn_args).detach().cpu()).double()
    ...
    output_pytorch = MSDeformAttnFunction.apply(*fn_args).detach().double().cpu()
    ...
    exp_time = speed_test(FlashDeformAttnFunction.apply, test_args, flash_fn_args, name="exp")
