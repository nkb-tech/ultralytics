...
def overlaped_channel_reverse(x, window_size, stride, pad, outC):
    ...
    out = torch.nn.functional.fold(
        x, output_size=(outC, 1), kernel_size=(window_size, 1), padding=(pad, 0), stride=(stride, 1)
    )
    return out
...
class CrossLayerSpatialAttention(nn.Module):
    def __init__(self, in_dim, layer_num=3, beta=1, num_heads=4, mlp_ratio=2, reduction=4):
        ...
        self.cpe = nn.ModuleList(
            nn.ModuleList([ConvPosEnc(dim=in_dim, k=3), ConvPosEnc(dim=in_dim, k=3)]) for i in range(layer_num)
        )
        ...
        self.qkv = nn.ModuleList(
            nn.Conv2d(in_dim, self.hidden_dim * 3, kernel_size=1, stride=1, padding=0) for i in range(layer_num)
        )

        mlp_hidden_dim = int(in_dim * mlp_ratio)
        self.mlp = nn.ModuleList(Mlp(in_features=in_dim, hidden_features=mlp_hidden_dim) for i in range(layer_num))
    ...
    def forward(self, x_list, extra=None):
        ...
        for i, x in enumerate(x_list):
            ...
            qkv_windows = qkv_windows.reshape(B, WmH * WmW, ws_i * ws_i, 3, self.num_heads, self.head_dim).permute(
                3, 0, 4, 1, 2, 5
            )
...
class CrossLayerChannelAttention(nn.Module):
    ...
    def forward(self, x_list, extra=None):
        ...
        for i, x in enumerate(x_list):
            ...
            ws_i, stride_i = self.window_sizes[i], self.stride_list[i]
            pad_i = math.ceil(
                (stride_i * (self.hidden_dim - 1.0) - (self.unshuffle_factor[i]) ** 2 * self.hidden_dim + ws_i) / 2.0
            )
            self.padding_list[i] = pad_i
            x = self.cpe[i][0](x)
            shortcut_list.append(x)
