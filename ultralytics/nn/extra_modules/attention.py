...
class TransNeXt_AggregatedAttention(nn.Module):
    def __init__(
        self, dim, input_resolution, sr_ratio=8, num_heads=8, window_size=3, qkv_bias=True, attn_drop=0.0, proj_drop=0.0
    ) -> None:
        super().__init__()
        ...
        relative_pos_index, relative_coords_table = get_relative_position_cpb(
            query_size=input_resolution, key_size=(20, 20), pretrain_size=input_resolution
        )

        self.register_buffer("relative_pos_index", relative_pos_index, persistent=False)
        self.register_buffer("relative_coords_table", relative_coords_table, persistent=False)
        self.attention = AggregatedAttention(
            dim, input_resolution, num_heads, window_size, qkv_bias, attn_drop, proj_drop, sr_ratio
        )

    def forward(self, x):
        B, _, H, W = x.size()
        x = x.flatten(2).transpose(1, 2)
        relative_pos_index = getattr(self, "relative_pos_index")
        relative_coords_table = getattr(self, "relative_coords_table")
...
class LocalWindowAttention(torch.nn.Module):
    ...
    def __init__(
        self, dim, key_dim=16, num_heads=4, attn_ratio=4, resolution=14, window_resolution=7, kernels=[5, 5, 5, 5]
    ):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
...
class ELA(nn.Module):
    def __init__(self, channels) -> None:
        ...
        self.pool_h = nn.AdaptiveAvgPool2d((None, 1))
        self.pool_w = nn.AdaptiveAvgPool2d((1, None))
        self.conv1x1 = nn.Sequential(nn.Conv1d(channels, channels, 1), nn.GroupNorm(16, channels), nn.Sigmoid())
