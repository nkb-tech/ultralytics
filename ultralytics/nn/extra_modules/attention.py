...
from ultralytics.nn.modules import Conv, MemoryEfficientSwish


__all__ = ['MPCA', 'MHSA', 'LSKBlock', 'EfficientAttention', 'MultiDilatelocalAttention', 'BiLevelRoutingAttention_nchw']
...
class MHSA(nn.Module):
    """
    MHSA Multi-Head-Self-Attention https://wuch15.github.io/paper/EMNLP2019-NRMS.pdf
    """
...
class LSKBlock(nn.Module):
    ...
    def forward(self, x):   
        ...
        sig = self.conv_squeeze(agg).sigmoid_()
...
class EfficientAttention(nn.Module):
    """CloAttention from https://arxiv.org/abs/2303.17803."""
...
class BiLevelRoutingAttention(nn.Module):
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        if self.auto_pad:
            ...
            x = torch.nn.functional.pad(x, (0, 0, # dim=-1
                          pad_l, pad_r, # dim=-2
                          pad_t, pad_b)) # dim=-3
...
def regional_routing_attention_torch(
    ...
    if auto_pad:
        ...
        if (q_pad_b > 0 or q_pad_r > 0):
            query = torch.nn.functional.pad(query, (0, q_pad_r, 0, q_pad_b)) # zero padding

        _, _, Hk, Wk = key.size()
        kv_pad_b = (kv_region_size[0] - Hk % kv_region_size[0]) % kv_region_size[0]
        kv_pad_r = (kv_region_size[1] - Wk % kv_region_size[1]) % kv_region_size[1]
        if (kv_pad_r > 0 or kv_pad_b > 0):
            key = torch.nn.functional.pad(key, (0, kv_pad_r, 0, kv_pad_b)) # zero padding
            value = torch.nn.functional.pad(value, (0, kv_pad_r, 0, kv_pad_b)) # zero padding
...
class BiLevelRoutingAttention_nchw(nn.Module):
    ...
    * different pooling behavior (count_include_pad=False)
    ...
    def forward(self, x: torch.Tensor, ret_attn_mask=False):
        ...
        q_r = torch.nn.functional.avg_pool2d(q.detach(), kernel_size=region_size, ceil_mode=True, count_include_pad=False)
        k_r = torch.nn.functional.avg_pool2d(k.detach(), kernel_size=region_size, ceil_mode=True, count_include_pad=False) # nchw
        q_r: torch.Tensor = q_r.permute(0, 2, 3, 1).flatten(1, 2) # n(hw)c
        k_r: torch.Tensor = k_r.flatten(2, 3) # nc(hw)
