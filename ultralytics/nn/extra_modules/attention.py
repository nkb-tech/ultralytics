...
class AttnMap(nn.Module):
    def __init__(self, dim):
        ...
        self.act_block = nn.Sequential(
            nn.Conv2d(dim, dim, 1, 1, 0), MemoryEfficientSwish(), nn.Conv2d(dim, dim, 1, 1, 0)
        )
...
class AttentionLePE(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0.0, proj_drop=0.0, side_dwconv=5):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        # NOTE scale factor was wrong in my original version, can set manually to be compat with prev weights
...
class DilateAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        k = (
            self.unfold(k)
            .reshape([B, d // self.head_dim, self.head_dim, self.kernel_size * self.kernel_size, H * W])
            .permute(0, 1, 4, 2, 3)
        )  # B,h,N,d,k*k
