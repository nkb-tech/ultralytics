...
from ..backbone.rmt import RelPos2d, RetBlock
from ..modules.block import *
from ..modules.conv import Conv, DSConv, DWConv, GhostConv, RepConv, autopad
...
class DyReLU(nn.Module):
    def __init__(
        ...
        self.fc = nn.Sequential(
            nn.Linear(inp, squeeze), nn.ReLU(inplace=True), nn.Linear(squeeze, self.oup * self.exp), h_sigmoid()
        )
        if use_spatial:
...
class DyDCNv2(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, stride=1, norm_cfg=dict(type="GN", num_groups=16, requires_grad=True)
    ):
        super().__init__()
...
class DyHeadBlockWithDCNV4(nn.Module):
    ...
    def __init__(
        ...
        self.spatial_conv_offset = nn.Conv2d(in_channels, self.offset_and_mask_dim, 1, padding=0, groups=1)
        self.scale_attn_module = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(in_channels, 1, 1),
            nn.ReLU(inplace=True),
...
class GSBottleneckns(GSBottleneck):
    ...
    def __init__(self, c1, c2, k=3, s=1, e=0.5):
        ...
        c_ = int(c2 * e)
        # for lighting
        self.conv_lighting = nn.Sequential(GSConvns(c1, c_, 1, 1), GSConvns(c_, c2, 3, 1, act=False))
...
class SCConv(nn.Module):
    ...
    def __init__(self, c1, c2, s=1, d=1, g=1, pooling_r=4):
        ...
        self.k2 = nn.Sequential(
            nn.AvgPool2d(kernel_size=pooling_r, stride=pooling_r), Conv(c1, c2, k=3, d=d, g=g, act=False)
        )
...
class Bottleneck_KW(Bottleneck):
    ...
    def __init__(
        self, c1, c2, wm=None, wm_name=None, shortcut=True, g=1, k=(3, 3), e=0.5
...
class C3_MSBlock(C3):
    def __init__(
        ...
        self.m = nn.Sequential(
            *(
                MSBlock(c_, c_, kernel_sizes, in_expand_ratio, mid_expand_ratio, layers_num, in_down_ratio)
                for _ in range(n)
            )
        )
...
class C2f_MSBlock(C2f):
    def __init__(
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            MSBlock(self.c, self.c, kernel_sizes, in_expand_ratio, mid_expand_ratio, layers_num, in_down_ratio)
            for _ in range(n)
        )
...
class MBConv(nn.Module):
    ...
    def forward(self, x):
        return (
            x + self.dropout(self.conv1(self.effective_se(self.conv_dw_1(self.conv_pw_1(x)))))
            if self.add
            else self.dropout(self.conv1(self.effective_se(self.conv_dw_1(self.conv_pw_1(x)))))
        )
...
class Bottleneck_DAttention(Bottleneck):
    ...
    def __init__(
        self, c1, c2, fmapsize, shortcut=True, g=1, k=(3, 3), e=0.5
    ):  # ch_in, ch_out, shortcut, groups, kernels, expand
        super().__init__(c1, c2, shortcut, g, k, e)
...
class C2f_Parc(C2f):
    def __init__(self, c1, c2, n=1, fmapsize=None, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            Bottleneck_ParC(self.c, self.c, fmapsize, shortcut, g, k=(3, 3), e=1.0) for _ in range(n)
        )
...
from ..backbone.UniRepLKNet import (
    GRNwithNHWC,
    NCHWtoNHWC,
    NHWCtoNCHW,
    SEBlock,
    ...
    get_bn,
    get_conv2d,
    merge_dilated_into_large_kernel,
)
...
class C2f_AggregatedAtt(C2f):
    def __init__(self, c1, c2, n=1, input_resolution=None, sr_ratio=None, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            Bottleneck_AggregatedAttention(self.c, self.c, input_resolution, sr_ratio, shortcut, g, k=(3, 3), e=1.0)
            for _ in range(n)
        )
...
class C3_AggregatedAtt(C3):
    def __init__(self, c1, c2, n=1, input_resolution=None, sr_ratio=None, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.Sequential(
            *(
                Bottleneck_AggregatedAttention(
                    c_, c_, input_resolution, sr_ratio, shortcut, g, k=((1, 1), (3, 3)), e=1.0
                )
                for _ in range(n)
            )
        )
...
class ELA_HSFPN(nn.Module):
    def __init__(self, in_planes, flag=True):
        ...
        self.conv1x1 = nn.Sequential(nn.Conv1d(in_planes, in_planes, 1), nn.GroupNorm(16, in_planes), nn.Sigmoid())
        self.flag = flag
...
class Bottleneck_SWC(Bottleneck):
    ...
    def __init__(
        self, c1, c2, kernel_size, shortcut=True, g=1, k=(3, 3), e=0.5
    ):  # ch_in, ch_out, shortcut, groups, kernels, expand
        super().__init__(c1, c2, shortcut, g, k, e)
        int(c2 * e)  # hidden channels
...
class C2f_SWC(C2f):
    def __init__(self, c1, c2, n=1, kernel_size=13, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            Bottleneck_SWC(self.c, self.c, kernel_size, shortcut, g, k=(3, 3), e=1.0) for _ in range(n)
        )
...
class DGCST(nn.Module):
    ...
    def __init__(self, c1, c2) -> None:
        ...
        self.conv2 = nn.Sequential(Conv(c2, c2, 1), Conv(c2, c2, 1))
...
class C2f_PKIModule(C2f):
    def __init__(
        ...
        super().__init__(c1, c2, n, True, g, e)
        self.m = nn.ModuleList(
            PKIModule(self.c, self.c, kernel_sizes, expansion, with_caa, caa_kernel_size, add_identity)
            for _ in range(n)
        )
...
class CSMHSA(nn.Module):
    def __init__(self, n_dims, heads=8):
        ...
        self.heads = heads
        self.query = nn.Sequential(nn.Upsample(scale_factor=2), nn.Conv2d(n_dims[0], n_dims[1], kernel_size=1))
...
class SFC_G2(nn.Module):
    def __init__(self, inc):
        ...
        self.conv_offset = nn.Sequential(
            Conv(hidc * 2, 64), nn.Conv2d(64, self.groups * 4 + 2, kernel_size=3, padding=1, bias=False)
        )
