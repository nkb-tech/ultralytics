...
class ConvFFN(BaseModule):
    ...
    def __init__(
        ...
        self.ffn_layers = nn.Sequential(
            ...
            BHWC2BCHW(),
            ConvModule(
                in_channels, hidden_channels, kernel_size=1, stride=1, padding=0, norm_cfg=norm_cfg, act_cfg=act_cfg
            ),
            ConvModule(
                hidden_channels,
                hidden_channels,
            ...
            ConvModule(
                hidden_channels, out_channels, kernel_size=1, stride=1, padding=0, norm_cfg=norm_cfg, act_cfg=act_cfg
            ),
            nn.Dropout(dropout_rate),
        )
...
class Stem(BaseModule):
    ...
    def __init__(
        ...
        self.down_conv = ConvModule(
            in_channels, hidden_channels, kernel_size=3, stride=2, padding=1, norm_cfg=norm_cfg, act_cfg=act_cfg
        )
        self.conv1 = ConvModule(
            hidden_channels, hidden_channels, kernel_size=3, stride=1, padding=1, norm_cfg=norm_cfg, act_cfg=act_cfg
        )
        self.conv2 = ConvModule(
            hidden_channels, out_channels, kernel_size=3, stride=1, padding=1, norm_cfg=norm_cfg, act_cfg=act_cfg
        )
...
class DownSamplingLayer(BaseModule):
    ...
    def __init__(
        ...
        self.down_conv = ConvModule(
            in_channels, out_channels, kernel_size=3, stride=2, padding=1, norm_cfg=norm_cfg, act_cfg=act_cfg
        )
...
class PKIStage(BaseModule):
    ...
    def __init__(
        ...
        ffn_kernel_size: int = 3,
        dropout_rate: float = 0.0,
        drop_path_rate: Union[float, list] = 0.0,
        layer_scale: Optional[float] = 1.0,
...
class PKINet(BaseModule):
    ...
    arch_settings = {
        ...
        "T": [
            [16, 32, 4, (3, 5, 7, 9, 11), (1, 1, 1, 1, 1), 0.5, 4.0, 3, 0.1, 1.0, True, 8.0, 5, True, True, 11],
        ...
        "S": [
            [32, 64, 4, (3, 5, 7, 9, 11), (1, 1, 1, 1, 1), 0.5, 4.0, 3, 0.1, 1.0, True, 8.0, 5, True, True, 11],
        ...
        "B": [
            [40, 80, 6, (3, 5, 7, 9, 11), (1, 1, 1, 1, 1), 0.5, 4.0, 3, 0.1, 1.0, True, 8.0, 5, True, True, 11],
