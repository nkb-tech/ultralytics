...
class SemanticMetrics(SimpleClass):  # upstream also mixes in DataExportMixin (absent in fork)
    ...
    # Форк держит метрики списком (по голове на таск), semantic — одиночный объект.
    def __getitem__(self, idx):
        """Return self for index 0 to satisfy the fork's multihead metrics[i] access."""
        if idx not in (0, -1):
            raise IndexError(f"SemanticMetrics has a single head, got index {idx}")
        return self
    ...
    def process(self, save_dir: Path = Path("."), plot: bool = False, on_plot=None) -> None:
    ...
    def _plot_iou_bars(self, save_dir, on_plot):
        ...
        """
        from ultralytics.utils.plotting import colors  # local import: plotting imports metrics
        import matplotlib.pyplot as plt
