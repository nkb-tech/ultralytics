...
class TaskAlignedAssigner(nn.Module):
    ...
    Attributes:
        ...
        iou_loss_fn (str): The IoU loss function to use for calculation.
    """
    ...
    def _forward(self, pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt):
        ...
        # Normalize
        norm_align_metric = self.get_norm_align_metric(align_metric, mask_pos, overlaps)
    ...
    def get_norm_align_metric(self, align_metric, mask_pos, overlaps):
        ...
        Args:
            align_metric (torch.Tensor): Alignment metric with shape (bs, n_max_boxes, h*w).
            mask_pos (torch.Tensor): Mask of positive anchor points with shape (bs, n_max_boxes, h*w).
            overlaps (torch.Tensor): Overlaps between anchor points and ground truth boxes with shape (bs, n_max_boxes, h*w).

        Returns:
            norm_align_metric (torch.Tensor): Normalized alignment metric with shape (bs, h*w, 1).
        """
        align_metric = mask_pos * align_metric
        pos_align_metrics = align_metric.amax(dim=-1, keepdim=True)  # b, max_num_obj
        pos_overlaps = (overlaps * mask_pos).amax(dim=-1, keepdim=True)  # b, max_num_obj
        norm_align_metric = (align_metric * pos_overlaps / (pos_align_metrics + self.eps)).amax(-2).unsqueeze(-1)
        return norm_align_metric
    ...
    def prepare_targets(self, target_gt_idx):
        """Prepare targets for the positive anchor points.

        Args:
            target_gt_idx (torch.Tensor): Target ground truth indices with shape (b, h*w).

        Returns:
            (torch.Tensor): Prepared target indices with shape (b, h*w).
        """
        batch_ind = torch.arange(end=self.bs, dtype=torch.int64, device=target_gt_idx.device)[..., None]
        target_gt_idx = target_gt_idx + batch_ind * self.n_max_boxes  # (b, h*w)
        return target_gt_idx

    def get_bboxes(self, gt_bboxes, target_gt_idx, fg_mask):
        """Get bounding boxes for the positive anchor points.

        Args:
            gt_bboxes (torch.Tensor): Ground truth bounding boxes with shape (b, max_num_obj, 4).
            target_gt_idx (torch.Tensor): Indices of the assigned ground truth objects for positive anchor points,
        ...
        Returns:
            (torch.Tensor): Target bounding boxes for positive anchor points with shape (b, h*w, 4).
        """
        if gt_bboxes.shape[1] == 0:
            return torch.zeros(self.bs, fg_mask.shape[1], gt_bboxes.shape[-1], device=gt_bboxes.device)
        return gt_bboxes.view(-1, gt_bboxes.shape[-1])[target_gt_idx]  # (b, h*w, 4)

    def get_scores(self, gt_labels, target_gt_idx, fg_mask, num_classes):
        """Compute target labels and target scores for the positive anchor points.
        ...
        if gt_labels.shape[1] == 0:
            return (
                torch.full((self.bs, fg_mask.shape[1]), fill_value=num_classes, device=gt_labels.device, dtype=gt_labels.dtype),
                torch.zeros((self.bs, fg_mask.shape[1], num_classes), device=gt_labels.device),
            )
        # Assigned target labels, (b, h*w)
        target_labels = gt_labels.long().flatten()[target_gt_idx].clamp_(0)  # (b, h*w)

        # Assigned target scores, (b, h*w, num_classes)
        ...
        fg_scores_mask = fg_mask[:, :, None].repeat(1, 1, num_classes)  # (b, h*w, num_classes)
        target_scores = torch.where(fg_scores_mask > 0, target_scores, 0)

        return target_labels, target_scores

    def get_targets(self, gt_labels, gt_bboxes, target_gt_idx, fg_mask):
        """Compute target labels, target bounding boxes, and target scores for the positive anchor points.

        This is a convenience method that combines prepare_targets, get_bboxes and get_scores.

        Args:
            gt_labels (torch.Tensor): Ground truth labels of shape (b, max_num_obj, 1).
            gt_bboxes (torch.Tensor): Ground truth bounding boxes of shape (b, max_num_obj, 4).
            target_gt_idx (torch.Tensor): Indices of the assigned ground truth objects for positive anchor points
                (before prepare_targets is called).
            fg_mask (torch.Tensor): A boolean tensor indicating the positive (foreground) anchor points.

        Returns:
            target_labels (torch.Tensor): Target labels for positive anchor points.
            target_bboxes (torch.Tensor): Target bounding boxes for positive anchor points.
            target_scores (torch.Tensor): Target scores for positive anchor points.
        """
    ...
    def select_candidates_in_gts(self, xy_centers, gt_bboxes, mask_gt, eps=1e-9):
        ...
        This method handles small detector boxes more efficiently by expanding very small boxes
        to a minimum size based on stride values.
        ...
        Notes:
            ...
            - Small boxes (width or height < smallest stride) are expanded to stride_val size
              to ensure at least one anchor point is selected for small objects.
        """
        # Handle small detector boxes by expanding them to minimum size
    ...
    def select_highest_overlaps(self, mask_pos, overlaps, n_max_boxes, align_metric):
        ...
        # Secondary topk filtering for additional refinement
...
class RotatedTaskAlignedAssigner(TaskAlignedAssigner):
    ...
    def select_candidates_in_gts(self, xy_centers, gt_bboxes, mask_gt):
        ...
        This method handles small rotated detector boxes more efficiently by expanding very small boxes
        to a minimum size based on stride values.
        ...
        """
        # Handle small rotated detector boxes by expanding them to minimum size
