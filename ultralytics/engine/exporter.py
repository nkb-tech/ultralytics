...
class Exporter:
        ...
        for m in model.modules():
            if isinstance(m, (Detect, RTDETRDecoder)):  # includes all Detect subclasses like Segment, Pose, OBB
                ...
                if not isinstance(m, RTDETRDecoder) and self.args.nms and (onnx or engine):
                    ...
                    post_detect_class.conf = self.args.conf
                    post_detect_class.iou = self.args.iou
    ...
    def export_onnx(self, prefix=colorstr("ONNX:")):
        ...
        if TORCH_2_6:  # dynamo param added in torch 2.6
        ...
        if self.args.int8:
            ...
            def transform_fn(data_item: dict) -> np.ndarray:
                """Quantization transform function."""
                data_item: torch.Tensor = data_item["img"] if isinstance(data_item, dict) else data_item
                assert data_item.dtype == torch.uint8, "Input image must be uint8 for the quantization preprocessing"
                im = data_item.numpy().astype(np.float32) / 255.0  # uint8 to fp16/32 and 0 - 255 to 0.0 - 1.0
                return {input_name: np.expand_dims(im, 0) if im.ndim == 3 else im}
            ...
            dataset = nncf.Dataset(dataloader, transform_fn)

            onnx_int8_model = nncf.quantize_with_accuracy_control(
                model=model_onnx,
                calibration_dataset=dataset,
                validation_dataset=dataset,
                validation_fn=validation_fn,
                subset_size=len(dataloader),
