...
class BaseTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        if self.args.teacher is not None:
            ...
            self.dist_loss = DistillationLoss(self.teacher, self.args.temperature, self.args.alpha, task=self.args.task)
    ...
    def _setup_train(self, world_size):
        ...
        # SyncBatchNorm
        if self.args.sync_bn and RANK != -1:
            self.model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(self.model)
            LOGGER.info(f"{colorstr('SyncBatchNorm:')} enabled")
            ...
            for i, batch in pbar:
                ...
                with autocast(self.amp):
                    ...
                    if self.args.teacher is not None:
                        ...
                        self.loss_items = torch.cat([self.loss_items, d_loss.unsqueeze(0)])
                        self.loss += d_loss
