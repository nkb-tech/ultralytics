...
class RGB2TIR:
    ...
    def __call__(self, labels):
        ...
        if is_pil:
            labels = Image.fromarray(infrared)
        else:
            labels["img"] = infrared
        ...
        if ALBU_AVAILABLE:
            try:
                ...
                composition_transforms = {
                    ...
                    "Sequential",
                    "SomeOf",
                }  # from https://albumentations.ai/docs/api_reference/core/composition

                def check_contains_spatial(transforms: Union[TransformsSeqType, TransformType]) -> bool:
                    if isinstance(transforms, (BasicTransform, BaseCompose)):
                        return check_contains_spatial([transforms])
                    else:
                        for transform in transforms:
                            if transform.__class__.__name__ in composition_transforms:
                                return check_contains_spatial(transform.transforms)
                            elif transform.__class__.__name__ in spatial_transforms:
                                return True
...
def classify_augmentations(
    ...
    Args:
        ...
        albu_quality_lower: ImageCompression quality lower
        albu_max_factor: ZoomBlur max_factor
        albu_clip_limit: CLAHE clip limit
        albu_brightness: ColorJitter brightness
        albu_contrast: ColorJitter contrast
        albu_saturation: ColorJitter saturation
        albu_hue: ColorJitter hue
