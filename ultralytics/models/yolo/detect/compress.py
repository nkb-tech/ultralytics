...
def get_pruner(opt, model, example_inputs):
    ...
    if opt.prune_method == "random":
        ...
        pruner_entry = partial(
            tp.pruner.BNScalePruner, reg=opt.reg, global_pruning=opt.global_pruning, group_lasso=True
        )
        ...
        imp = tp.importance.GroupNormImportance(p=2)
        pruner_entry = partial(
            tp.pruner.GrowingRegPruner, reg=opt.reg, delta_reg=opt.delta_reg, global_pruning=opt.global_pruning
        )
...
def linear_trans(epoch, epochs, reg, reg_ratio):
    return (1 - epoch / (epochs - 1)) * (reg - reg_ratio) + reg_ratio
...
class DetectionCompressor(BaseTrainer):
    ...
    def save_model(self):
        ...
        if self.best_sl[f"{self.sparsity_ratio:.3f}"] == self.fitness:
            torch.save(
                ...
                self.wdir / f"best_sl_{self.sparsity_ratio:.3f}.pt",
            )
    ...
    def sparsity_learning(self, ckpt, world_size, prune):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            plt.close("all")
            LOGGER.info(
                f"epoch:{epoch} sparsity_ratio:{self.sparsity_ratio:.5f} bn_weight_1:{bn_weight_percent[0]:.10f} bn_weight_5:{bn_weight_percent[1]:.8f} bn_weight_10:{bn_weight_percent[2]:.8f}\nbn_weight_25:{bn_weight_percent[3]:.5f} bn_weight_50:{bn_weight_percent[4]:.5f} bn_weight_75:{bn_weight_percent[5]:.5f}"
            )
        ...
        best_sl_model = self.wdir / f"best_sl_{sl}.pt"
    ...
    def setup(rank, world_size):
        ...
        dist.init_process_group("yolo_sparse", rank=rank, world_size=world_size)
...
class DetectionFinetune(DetectionTrainer):
    def get_model(self, cfg=None, weights=None, verbose=True):
        # Return a YOLO detection model.#
        model = torch.load(self.args.model, map_location=self.device)
        model = model["ema" if model.get("ema") else "model"].float()
