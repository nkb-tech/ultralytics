...
class DetectionValidator(BaseValidator):
    ...
    def _tensor_to_pred_dict(self, x: torch.Tensor, end2end: bool | None = None) -> dict[str, torch.Tensor]:
        ...
        Args:
            x: NMS output tensor.
            end2end: Optional override of ``self.end2end``. Useful for flows like SAHI
                where predictions are aggregated in multitask layout even when the
                model itself is end2end.
        """
        dev = x.device
        n = x.shape[0]
        out = {"bboxes": x[:, :4]}
        is_end2end = self.end2end if end2end is None else end2end
        if is_end2end:
    ...
    def _process_complete_image(self, img_key):
        ...
        if len(aggregated) == 0:
            ...
            # reconstructed scores), so parse them as multitask regardless of model.end2end.
            aggregated_pred = self._tensor_to_pred_dict(nms_results[0], end2end=False)
    ...
    def print_results(self) -> None:
        ...
        # Per-size summary (Standard AP s/m/l): reports mAP50/mAP50-95 for each size bucket.
        for i in range(self.num_tasks):
            label = f"task{i}" if self.num_tasks > 1 else "all"
            parts = []
            for sz in SIZE_RANGES:
                sm = self.size_metrics[sz][i]
                parts.append(f"{sz}={sm.box.map50:.3f}/{sm.box.map:.3f}")
            LOGGER.info(f"{'':>22s} {label} Standard mAP50/50-95 s/m/l: {', '.join(parts)}")
