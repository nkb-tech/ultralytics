...
def transfer_weights_c2f_v2_to_c2f(c2f_v2, c2f):
    ...
    state_dict["cv1.conv.weight"]
    new_cv1 = Conv(
        c1=state_dict_v2["cv0.conv.weight"].size()[1],
    ...
    state_dict["cv1.conv.weight"] = torch.cat(
        [state_dict_v2["cv0.conv.weight"], state_dict_v2["cv1.conv.weight"]], dim=0
    )

    # Transfer cv1 batchnorm weights and buffers from C2f to cv0 and cv1 in C2f_v2
    for bn_key in ["weight", "bias", "running_mean", "running_var"]:
        state_dict[f"cv1.bn.{bn_key}"] = torch.cat(
            [state_dict_v2[f"cv0.bn.{bn_key}"], state_dict_v2[f"cv1.bn.{bn_key}"]], dim=0
        )
