...
def _scalar(v):
    """Форк заворачивает nc/names в список для multihead, semantic ждёт скаляр."""
...
class SemanticSegmentationTrainer(DetectionTrainer):
    ...
    def get_dataset(self):
        ...
        # Форковый super() отдаёт кортеж (train, val), dict кладёт в self.data — работаем с ним.
    ...
    def get_model(self, cfg=None, weights=None, verbose: bool = True):
        ...
        # ch=3: ключа "channels" в форковом self.data нет.
    ...
    def get_validator(self):
        ...
        # Порядок обязан совпадать с тензором из SemanticSegmentationLoss.forward().
    ...
    def preprocess_batch(self, batch):
        ...
        # Родитель под multi_scale интерполирует только img — маска бы разъехалась.
        batch["img"] = batch["img"].to(self.device, non_blocking=True).float() / 255
        if "semantic_mask" in batch:
            batch["semantic_mask"] = batch["semantic_mask"].to(self.device, non_blocking=True).long()
        # _do_train читает batch["cls"].shape[0] как размер батча — в semantic его нет.
        # Статистики BN претрейна считались на другом батче/разрешении — пересчёт на наших
        # данных роняет метрику. momentum=0 замораживает running_mean/var, оставляя слой обучаемым.
        if str(self.args.model).endswith(".pt"):
            for m in self.model.modules():
                if isinstance(m, torch.nn.BatchNorm2d):
                    m.momentum = 0.0
        batch.setdefault("cls", torch.zeros(batch["img"].shape[0], 1, device=self.device))
        return batch

    # Родительские версии лезут в batch["cls"]/["bboxes"] и в детекционные колонки CSV.
    ...
    def validate(self):
        """Валидация идёт под inference_mode, и созданный там criterion ломает backward."""
